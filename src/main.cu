...
int main() {
    ...
    Sphere sphere(3.0, Eigen::Vector3d{0, 0, 0});
    ...
    std::vector<Body> bodies{world, light, body};
    for(auto & i : room) {
        bodies.push_back(i);
    }

    const Eigen::Vector3d camOrg{0, 0, 7.0};
    const Camera camera(
            camOrg,
            body.getSphere().center - camOrg,
            256, 1.0, 39, 0.6, (body.getSphere().center - camOrg).norm(), 2.0, 20
    );
